...
%%% Generated by abnfc_gen on 2013-03-30 18:32:06
...
-export([alternation_generator/1, 'bin-val_generator'/1,
	 'c-nl_generator'/1, 'c-wsp_generator'/1,
	 'char-val_generator'/1, comment_generator/1,
	 concatenation_generator/1, 'cs-char-val_generator'/1,
	 'dec-val_generator'/1, 'defined-as_generator'/1,
	 element_generator/1, elements_generator/1, generator/1,
	 group_generator/1, 'hex-val_generator'/1,
	 'num-val_generator'/1, option_generator/1,
	 'prose-val_generator'/1, repeat_generator/1,
	 repetition_generator/1, rule_generator/1,
	 rulelist_generator/1, rulename_generator/1]).
...
generator(rulelist) -> rulelist_generator(0);
generator(rule) -> rule_generator(0);
generator(rulename) -> rulename_generator(0);
generator('defined-as') -> 'defined-as_generator'(0);
generator(elements) -> elements_generator(0);
generator('c-wsp') -> 'c-wsp_generator'(0);
generator('c-nl') -> 'c-nl_generator'(0);
generator(comment) -> comment_generator(0);
generator(alternation) -> alternation_generator(0);
generator(concatenation) -> concatenation_generator(0);
generator(repetition) -> repetition_generator(0);
generator(repeat) -> repeat_generator(0);
generator(element) -> element_generator(0);
generator(group) -> group_generator(0);
generator(option) -> option_generator(0);
generator('char-val') -> 'char-val_generator'(0);
generator('cs-char-val') -> 'cs-char-val_generator'(0);
generator('num-val') -> 'num-val_generator'(0);
generator('bin-val') -> 'bin-val_generator'(0);
generator('dec-val') -> 'dec-val_generator'(0);
generator('hex-val') -> 'hex-val_generator'(0);
generator('prose-val') -> 'prose-val_generator'(0).
...
alternation_generator(Depth) when Depth >= 0 ->
    ?'LET'(V11,
	   (?'LET'(V10,
		   ('__seq_generator'([?'SIZED'(V1,
						(resize(erlang:round(math:sqrt(V1)),
							concatenation_generator(Depth + 1)))),
				       ?'LET'(V9,
...
'bin-val_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V27,
	   (?'LET'(V26,
		   ('__seq_generator'([?'LET'(V3,
					      (oneof([?'LET'(V1, 98, V1),
				       ...
				       ?'LET'(V25,
					      ('__repeat_generator'(0, 1,
								    ?'LET'(V24,
									   ('__terminals_oneof_generator'(Depth,
													  10,
													  [?'LET'(V19,
														  ('__repeat_generator'(1,
																	infinity,
																	?'LET'(V18,
																	       ('__seq_generator'([?'LET'(V15,
																					  46,
																					  V15),
																				   ?'LET'(V17,
																					  ('__repeat_generator'(1,
																								infinity,
																								?'SIZED'(V16,
																									 (resize(erlang:round(math:sqrt(V16)),
																										 'BIT_generator'(Depth
																												   +
																												   1)))))),
																					  V17)])),
...
'c-wsp_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V10,
	   (?'LET'(V9,
		   ('__terminals_oneof_generator'(Depth, 10,
						  [?'SIZED'(V5,
...
'char-val_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V8,
	   (?'LET'(V7,
		   ('__seq_generator'([?'SIZED'(V1,
						(resize(erlang:round(math:sqrt(V1)),
							'DQUOTE_generator'(Depth + 1)))),
				       ?'LET'(V5,
					      ('__repeat_generator'(0, infinity,
								    ?'LET'(V4,
									   (oneof([?'LET'(V2,
...
comment_generator(Depth) when Depth >= 0 ->
    ?'LET'(V10,
	   (?'LET'(V9,
		   ('__seq_generator'([?'LET'(V1, 59, V1),
				       ...
				       ?'SIZED'(V8,
						(resize(erlang:round(math:sqrt(V8)),
							'CRLF_generator'(Depth + 1))))])),
		   V9)),
	   V10).

concatenation_generator(Depth) when Depth >= 0 ->
    ?'LET'(V8,
	   (?'LET'(V7,
		   ('__seq_generator'([?'SIZED'(V1,
						(resize(erlang:round(math:sqrt(V1)),
							repetition_generator(Depth + 1)))),
				       ?'LET'(V6,
					      ('__repeat_generator'(0, infinity,
								    ?'LET'(V5,
									   ('__seq_generator'([?'LET'(V3,
...
'cs-char-val_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V8,
	   (?'LET'(V7,
		   ('__seq_generator'([?'SIZED'(V1,
						(resize(erlang:round(math:sqrt(V1)),
							'SQUOTE_generator'(Depth + 1)))),
				       ?'LET'(V5,
					      ('__repeat_generator'(0, infinity,
								    ?'LET'(V4,
									   (oneof([?'LET'(V2,
...
'dec-val_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V27,
	   (?'LET'(V26,
		   ('__seq_generator'([?'LET'(V3,
					      (oneof([?'LET'(V1, 100, V1),
...
'defined-as_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V17,
	   (?'LET'(V16,
...
element_generator(Depth) when Depth >= 0 ->
    ?'LET'(V14,
	   (?'LET'(V13,
		   ('__terminals_oneof_generator'(Depth, 10,
						  [?'SIZED'(V8,
							    (resize(erlang:round(math:sqrt(V8)),
								    rulename_generator(Depth +
											 1)))),
						   ?'SIZED'(V9,
							    (resize(erlang:round(math:sqrt(V9)),
								    'char-val_generator'(Depth
											   +
											   1)))),
						   ?'SIZED'(V10,
							    (resize(erlang:round(math:sqrt(V10)),
								    'cs-char-val_generator'(Depth
											      +
											      1)))),
						   ?'SIZED'(V11,
							    (resize(erlang:round(math:sqrt(V11)),
								    'num-val_generator'(Depth +
											  1)))),
						   ?'SIZED'(V12,
							    (resize(erlang:round(math:sqrt(V12)),
								    'prose-val_generator'(Depth
											    +
											    1))))],
						  [?'SIZED'(V1,
							    (resize(erlang:round(math:sqrt(V1)),
						   ...
						   ?'SIZED'(V2,
							    (resize(erlang:round(math:sqrt(V2)),
								    group_generator(Depth +
										      1)))),
						   ?'SIZED'(V3,
							    (resize(erlang:round(math:sqrt(V3)),
								    option_generator(Depth +
										       1)))),
						   ?'SIZED'(V4,
							    (resize(erlang:round(math:sqrt(V4)),
								    'char-val_generator'(Depth
...
elements_generator(Depth) when Depth >= 0 ->
    ?'LET'(V5,
	   (?'LET'(V4,
		   ('__seq_generator'([?'SIZED'(V1,
						(resize(erlang:round(math:sqrt(V1)),
							alternation_generator(Depth + 1)))),
				       ?'LET'(V3,
					      ('__repeat_generator'(0, infinity,
								    ?'SIZED'(V2,
									     (resize(erlang:round(math:sqrt(V2)),
...
group_generator(Depth) when Depth >= 0 ->
    ?'LET'(V9,
	   (?'LET'(V8,
...
'hex-val_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V27,
	   (?'LET'(V26,
		   ('__seq_generator'([?'LET'(V3,
					      (oneof([?'LET'(V1, 120, V1),
...
'num-val_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V10,
	   (?'LET'(V9,
		   ('__seq_generator'([?'LET'(V1, 37, V1),
				       ?'LET'(V8,
					      ('__terminals_oneof_generator'(Depth, 10,
									     [?'SIZED'(V5,
										       (resize(erlang:round(math:sqrt(V5)),
											       'bin-val_generator'(Depth
														     +
														     1)))),
									      ?'SIZED'(V6,
										       (resize(erlang:round(math:sqrt(V6)),
											       'dec-val_generator'(Depth
														     +
														     1)))),
									      ?'SIZED'(V7,
										       (resize(erlang:round(math:sqrt(V7)),
											       'hex-val_generator'(Depth
														     +
														     1))))],
									     [?'SIZED'(V2,
										       (resize(erlang:round(math:sqrt(V2)),
...
option_generator(Depth) when Depth >= 0 ->
    ?'LET'(V9,
	   (?'LET'(V8,
...
'prose-val_generator'(Depth) when Depth >= 0 ->
    ?'LET'(V8,
	   (?'LET'(V7,
...
repeat_generator(Depth) when Depth >= 0 ->
    ?'LET'(V18,
	   (?'LET'(V17,
		   ('__terminals_oneof_generator'(Depth, 10,
						  [?'LET'(V14,
						   ...
						   ?'LET'(V16,
							  ('__repeat_generator'(1, infinity,
										?'SIZED'(V15,
											 (resize(erlang:round(math:sqrt(V15)),
												 'DIGIT_generator'(Depth
														     +
														     1)))))),
							  V16)],
...
repetition_generator(Depth) when Depth >= 0 ->
    ?'LET'(V5,
	   (?'LET'(V4,
		   ('__seq_generator'([?'LET'(V2,
					      ('__repeat_generator'(0, 1,
								    ?'SIZED'(V1,
									     (resize(erlang:round(math:sqrt(V1)),
										     repeat_generator(Depth
													+
													1)))))),
					      V2),
				       ?'SIZED'(V3,
						(resize(erlang:round(math:sqrt(V3)),
							element_generator(Depth + 1))))])),
		   V4)),
	   V5).

rule_generator(Depth) when Depth >= 0 ->
    ?'LET'(V14,
	   (?'LET'(V13,
		   ('__seq_generator'([?'SIZED'(V1,
						(resize(erlang:round(math:sqrt(V1)),
							rulename_generator(Depth + 1)))),
				       ?'SIZED'(V2,
						(resize(erlang:round(math:sqrt(V2)),
							'defined-as_generator'(Depth + 1)))),
				       ?'SIZED'(V3,
						(resize(erlang:round(math:sqrt(V3)),
							elements_generator(Depth + 1)))),
				       ?'LET'(V11,
					      ('__repeat_generator'(0, 1,
								    ?'LET'(V10,
									   ('__seq_generator'([?'LET'(V5,
				       ...
				       ?'SIZED'(V12,
						(resize(erlang:round(math:sqrt(V12)),
							'c-nl_generator'(Depth + 1))))])),
		   V13)),
	   V14).

rulelist_generator(Depth) when Depth >= 0 ->
    ?'LET'(V13,
	   (?'LET'(V12,
		   ('__repeat_generator'(1, infinity,
					 ?'LET'(V11,
						('__terminals_oneof_generator'(Depth, 10,
									       [?'SIZED'(V6,
											 (resize(erlang:round(math:sqrt(V6)),
												 rule_generator(Depth
														  +
														  1)))),
										?'LET'(V10,
										       ('__seq_generator'([?'LET'(V8,
														  ('__repeat_generator'(0,
																	infinity,
																	?'SIZED'(V7,
																		 (resize(erlang:round(math:sqrt(V7)),
...
rulename_generator(Depth) when Depth >= 0 ->
    ?'LET'(V13,
	   (?'LET'(V12,
		   ('__seq_generator'([?'SIZED'(V1,
						(resize(erlang:round(math:sqrt(V1)),
							'ALPHA_generator'(Depth + 1)))),
				       ?'LET'(V11,
					      ('__repeat_generator'(0, infinity,
								    ?'LET'(V10,
									   ('__terminals_oneof_generator'(Depth,
